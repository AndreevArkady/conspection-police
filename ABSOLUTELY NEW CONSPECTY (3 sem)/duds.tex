...
\begin{defn}
  \se{неоднородная линейная система}{Неоднородная линейная система}
...
\begin{defn}
  \se{Логарифм матрицы}{Логарифм матрицы} $\log(B) $ -- такая матрица $A$, что $e^A = B$, так как $e^A$ всегда обратима, необходимо, чтобы $\det B \neq 0$
\end{defn}
...
\begin{defn}
  \se{Матрица монодромии}{Матрица монодромии} такая матрица 
  $B$, что $\Phi(t+\omega) = \Phi(t)B$
  
   \se{мультипликаторы}{мультипликаторы} -- собственные числа матрицы монодромии
\end{defn}
\begin{stat}
...
\end{proof}
\begin{theorem}
  \se{Теорема о мультипликаторах}{о мультипликаторах}
...
\section{Линейные дифференциальные уравнения высших порядков}

\begin{defn}
  \se{Линейное дифференциальное уравнение порядка $n$}{Линейное дифференциальное уравнение порядка $n$}

  \[x^{(n)} + a_1(t)x^{(n-1)} + \ldots + a_n(t)x = b(t)\]

  Коэффициенты $a_1, \ldots, a_n, b \in C(I), \ I $-- открытый промежуток $\mathbb{R}$

  
\end{defn}
\begin{defn}
  Решением уравнения называется функция $x(t)$, определенная на $(\alpha, \beta) \subset I:$ и удовлетворяющая следующим свойствам:
  \begin{enumerate}
  \item $\exists\ \dot x, \ldots, x^{(n)}$ на $(\alpha,\beta)$
  \item $x^{(n)}(t) + \ldots + a_n(t)x(t) \equiv b(t)$
  \end{enumerate}
\end{defn}
\begin{defn}
  Решение задачи Коши

  Фиксируем начальные данные $t_0 \in I$ и числа $x_0,\dot x_0, \ldots, x_0^{(n-1)}$

  Решение задачи Коши с этими начальными данными -- решение $x(t)$ на $(\alpha,\beta)$, такое что

  $$x(t_0) = x_0, \ldots, x^{(n-1)}(t_0) = x_0^{(n-1)}$$
\end{defn}
